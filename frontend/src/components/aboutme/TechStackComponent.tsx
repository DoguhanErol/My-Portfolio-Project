...
const TechStackComponent:React.FC<TProps> = (props:TProps) => {
  ...
  React.useEffect(() => {
    // props değiştiğinde yeni bir sorgu yapmak için useQuery hookunu yeniden çağırıyoruz
    techStacks.refetch();
  }, [props.TechStackCategoryId]);
    ...
    <>
      <section className="flex flex-wrap -m-2">
        {techStacks.data && techStacks.data.length > 0 && techStacks.data.map((techStack) => (
          <article className="p-2 lg:w-1/3 md:w-1/2 w-full" key={techStack.id}>
            <div className="h-full flex items-center border-gray-800 border p-4 rounded-lg">
              ...
              <div className="flex-grow">
                ...
                <p className="text-gray-600">{techStack.tech_stack_info}</p>
              </div>
            </div>
          </article>
        ))}
